...
func (p *packetPackerLegacy) writeAndSealPacket(
	...
	raw = raw[0:buffer.Len()]
        fmt.Printf("+++0+++++++in writeAndSealPacket ++++++++%x\n",raw)
	_ = sealer.Seal(raw[payloadStartIndex:payloadStartIndex], raw[payloadStartIndex:], header.PacketNumber, raw[:payloadStartIndex])
        fmt.Printf("+++1+++++++in writeAndSealPacket ++++++++%x\n",raw)
        if string(raw[payloadStartIndex:payloadStartIndex+4]) == "TEXT"{
	   raw = raw[0 : buffer.Len()+4]
        }else{
	    raw = raw[0 : buffer.Len()+sealer.Overhead()]
        }
        //fmt.Printf("++++++++++in writeAndSealPacket ++++++++%x\n",raw)
